...
func TestRoll5Dice(t *testing.T) {
	t.Run("returns a 5-digit number", testRoll5DiceRange)
	t.Run("each digit is between 1 and 6", testRoll5DiceDigitRange)
	t.Run("produces different results across multiple calls", testRoll5DiceVariety)
	t.Run("concurrent calls don't error", testRoll5DiceConcurrent)
	t.Run("no invalid digits like 0, 7, 8, or 9", testRoll5DiceNoInvalidDigits)
}

func testRoll5DiceRange(t *testing.T) {
...
func testRoll5DiceDigitRange(t *testing.T) {
	result, err := words.Roll5Dice()
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func testRoll5DiceVariety(t *testing.T) {
...
func testRoll5DiceConcurrent(t *testing.T) {
	t.Parallel()

	for i := range 1000 { // Reduced from 100000 to keep test reasonable
...
func testRoll5DiceNoInvalidDigits(t *testing.T) {
	// Run multiple times to increase confidence
	for attempt := range 1000 { // Reduced from 100000 to keep test reasonable
		result, err := words.Roll5Dice()
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
